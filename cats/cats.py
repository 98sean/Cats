...
def pick(paragraphs, select, k):
    ...
    if k < len(thinning_out(paragraphs, select)):
...
def about(topic):
    ...
    def retrun_function(my_paragraph):
        ...
        for my_word in topic:
            if my_word in my_paragraph_list:
                return True
...
def accuracy(typed, source):
    ...
    matched = 0
    cycle = min(total_typed, total_source)  
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        lowest_diff = diff_function(typed_word, word_list[0], limit)
        result = word_list[0]

        for i in range(1, len(word_list)):
            if diff_function(typed_word, word_list[i], limit) < lowest_diff:
                lowest_diff = diff_function(typed_word, word_list[i], limit)
                result = word_list[i]
...
def feline_fixes(typed, source, limit):
    ...
    #    if len(typed) <= index or len(source) <= index:
    #        return 0
    ...
    #        return limit + 1
    #    else:
    #        return count + helper_fixes(typed, source, index + 1, iteration, limit)

    #return helper_fixes(typed, source, 0, limit, limit) + abs(len(typed) - len(source))
...
def minimum_mewtations(start, goal, limit):
    ...
    if not start or not goal:
        ...
        if start[0] == goal[0]:
            ...
            substitution = 1 + minimum_mewtations(start[1:], goal[1:], limit - 1)
            return min(add, remove, substitution)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i] and breaking :
            count += 1       
        else:
            breaking = False
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            inner_times += [times_per_player[i][j + 1] - times_per_player[i][j]]
...
def fastest_words(match):
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            current_word = get_word(match, j)
            minimum_time = time(match, i, j)
            minimum_player = i

            for k in player_indices:
                if time(match, k, j) < minimum_time:
                    minimum_time = time(match, k, j)
                    minimum_player = k
                elif time(match, k, j) == minimum_time and k < i:
                    minimum_player = k
